...
def main():
    ...
    command += "python train.py --log_dir {log_dir} --p {{p}} --dim {{dim}} --width {{width}} --depth {{depth}} {args}".format(
        log_dir=args.log_dir, args=args.args)

    running = []

    for p, dim, width, depth in product(args.p, args.dim, args.width, args.depth):
        ...
        cmd = command.format(p=p, dim=dim, width=width, depth=depth)
